...
class Channel(PartialChannel):
    ...
    # type:ignore | fills in info not available in a PartialChannel object
    async def updateChannel(self, name: Union[str, None] = None, topic: Union[str, None] = "", nsfw: Union[bool, None] = None, bitrate: Union[int, None] = None, user_limit: Union[int, None] = None) -> 'Channel':
        if not name:
            name = self.name
        if self.type == 0:
            if topic == "":
