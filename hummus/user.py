...
class Member:
    def __init__(self, data, guild_id, instance, roles: list[Role] | None = None):
    ...
    async def hasChannelPermission(self, channel: Channel, permission: str) -> bool:
        if await self.hasPermission("administrator"):
            return True
        if isinstance(channel, str):  # in case someone does a thingy
            for guild in self.instance.guilds:
    ...
    async def send_friend_request(self) -> None:
        return await self.instance.http.add_friend(self.id)
    ...
    async def setRoles(self, roles: list[Role | str]) -> None:
        roles = [role.id if isinstance(role, Role) else role for role in roles]
        return await self.instance.http.update_guild_member(self.guild_id, self.id, roles=roles)

    async def removeRoles(self, roles: list[Role | str]):
        role_ids: list[str] = [role.id for role in self.roles]
        for role in roles:
            if isinstance(role, Role):
                role_ids.remove(role.id)
            else:
                role_ids.remove(role)
        return await self.instance.http.update_guild_member(self.guild_id, self.id, roles=role_ids)

    async def addRoles(self, roles: list[Role | str]) -> None:
        role_ids: set[str] = {role.id for role in self.roles}
        for role in roles:
            role_ids.add(role.id if isinstance(role, Role) else role)
        return await self.instance.http.update_guild_member(self.guild_id, self.id, roles=list(role_ids))
...
class User:
    def __init__(self, data, instance, guild_id=None):
        ...
        self.bot: bool | None = data.get('bot')  # type: ignore[x]
    ...
    async def getGuildMember(self, guild_id: str | None = None) -> Member:
    ...
    async def setRoles(self, roles: list[Role | str]) -> None:
        if not self.guild_id:
            raise Exception("This user was not found in a guild.")
        role_ids = [role.id if isinstance(role, Role) else role for role in roles]
        return await self.instance.http.update_guild_member(self.guild_id, self.id, roles=role_ids)

    async def removeRoles(self, roles: list[Role | str]) -> None:
        if not self.guild_id:
            raise Exception("This user was not found in a guild.")
        m = await self.getGuildMember()
        role_ids = [role.id for role in m.roles]
        for role in roles:
            role_ids.remove(role.id if isinstance(role, Role) else role)
        return await self.instance.http.update_guild_member(self.guild_id, self.id, roles=role_ids)

    async def addRoles(self, roles: list[Role | str]) -> None:
        if not self.guild_id:
            raise Exception("This user was not found in a guild.")
        g = await self.getGuildMember()
        role_ids = [role.id for role in g.roles]
        for role in roles:
            role_ids.append(role.id if isinstance(role, Role) else role)
        return await self.instance.http.update_guild_member(self.guild_id, self.id, roles=role_ids)
